...
### Recover a Failed Cross-Sandbox Clone
...
NemoClaw also keeps the clone's exact route and create-attempt record.
Run the printed selector command.
Do not retry the restore while the selector reports a sandbox or OpenShell cannot confirm the result.
Retry only when the selector and destination name are absent and the retained route reservation still matches the snapshot source.
If NemoClaw reports a route mismatch, preserve the recovery record and stop.
NemoClaw removes only the matching retained route reservation before it starts a new create attempt.
If the selector identifies a sandbox, resolve that retained sandbox through its owning gateway before you retry.

If NemoClaw confirms the destination identity but cannot register or publish it, the restore exits nonzero before state restore.
If the failure reports that the pending clone changed during registration, NemoClaw preserves the current registry row.
If the pending row and live destination still match this restore, retry the same command without `--force` so NemoClaw can reconcile them.
If you cannot prove that match, preserve the error and pending row for maintainer review.
Do not use `--force` or delete the destination by name when ownership is uncertain.
If the failure confirms an unregistered destination, run the owner-scoped deletion command that it prints:
...
Use this name-based deletion only when the error confirms that OpenShell created an unregistered destination. Do not use it for an ambiguous create result.
...
The agent-native `.deepagents/.mcp.json` is excluded from ordinary file copying because MCP is carried through the bounded source handoff.
For a native-MCP runtime, NemoClaw carries managed entries from the agent source through the bounded rebuild or restore handoff.
User-authored `hooks.json` is included and restored because managed interactive sessions support native Deep Agents Code hooks; review its executable commands before restoring a snapshot from another workspace.
NemoClaw recreates the current inference route headers, `models` and `update` tables, and provider credentials from host-side onboarding and OpenShell provider state during rebuild.

### Restore Managed Deep Agents Configuration

For a NemoClaw-managed Deep Agents image, NemoClaw restores the allowlisted UI and thread preferences, native interpreter settings, and native startup approval mode from the previous `config.toml` when their values pass validation. Unknown, runtime-controlled, privileged, and security-sensitive backup keys are dropped instead of replacing freshly generated settings on that managed path.

Before it restores snapshot files for a native-MCP runtime, NemoClaw verifies the current agent source and writes the native `.mcp.json` entry atomically. A legacy `.nemoclaw-mcp.json` source must be migrated first. If the native path `/sandbox/.deepagents/.mcp.json` is a directory, restore leaves snapshot files unchanged and prints a recovery command.
Run that command to move the directory into a new recovery location.
The command prints the selected location and does not replace an existing recovery file or directory.
Then rerun the same snapshot restore command.
...
If a late check fails, NemoClaw does not delete the existing sandbox.
