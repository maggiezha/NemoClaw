...
Port `11434` must be free on `127.0.0.1` and on the Portable host-gateway listener IP before inference create.
Stop a host Ollama service or leftover Portable Ollama container on those published addresses first.
...
When `nvidia-smi` reports multiple plausible NVIDIA GPU rows on WSL ARM64, NemoClaw runs the pinned CUDA workload once for every provider-visible GPU UUID and accepts the proof only when every reported device name and capacity matches.
...
Missing, malformed, mismatched, or insufficient proof capacity retains the `qwen3.5:9b` fallback.
