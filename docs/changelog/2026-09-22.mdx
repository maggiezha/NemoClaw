...
- `nemoclaw config export` now emits the supported `nemoclaw.nvidia.com/v1alpha1` shape for hosted single-agent OpenClaw, Hermes, and Deep Agents sandboxes.
  OpenClaw and Hermes exports use the singular `sandbox.agent` field and omit the target-managed image, while imported corporate CA state no longer blocks migration output.
