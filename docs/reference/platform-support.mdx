...
| Windows WSL2 | Docker Desktop (WSL backend), Qualified rootless Podman | Tested with limitations | P1 | No | Windows Express acceptance or the required non-interactive third-party software acceptance is the explicit opt-in to this Experimental surface. Windows Express and non-interactive onboarding automatically select managed llama.cpp when no provider is requested or recorded and `NEMOCLAW_MODEL` is unset. With `NEMOCLAW_LLAMACPP_RECIPE` unset, onboarding selects Qwen 3.6; a compatible explicit recipe selects that managed recipe. Selection requires a bounded CUDA proof to validate one GPU whose normalized identity is either `NVIDIA RTX Spark N1X` or `NVIDIA RTX Spark N1X (6144-core Blackwell RTX GPU)` on Linux ARM64 WSL, the local-default Docker authority to be active, and the 48,000 MiB Docker and GPU memory, driver, NVIDIA integration, and Docker Desktop GPU-passthrough checks to pass. The Windows chassis model does not participate in managed llama.cpp selection. Ordinary interactive onboarding can select another listed provider. Other Windows WSL Express installs use WSL-local Ollama, which can use Docker Desktop or the qualified rootless Podman provider. On a Windows WSL N1x, the selected provider runs the same bounded CUDA workload and capacity query before the installed-model ranking can select qwen3.6:35b. Windows-host Ollama remains Docker Desktop-only. Dedicated N1x WSL CI and physical end-to-end validation are pending. |
...
| Sandboxed execution | Tested with limitations | Landlock, seccomp, network namespace isolation, no-new-privileges, privilege dropping, and process limits (nproc 512 at `scripts/lib/sandbox-rlimits.sh:8`) are tested and on by default. OpenShell owns capability enforcement for managed entrypoints, exec processes, and connect shells; those paths do not call NemoClaw capability handling. Direct container runtimes that explicitly override the image user to root retain the existing compatibility drop and required `setpriv` transition. Direct container launches must configure capabilities at the container runtime. |
...
| Native rootless Podman on Linux | Tested with limitations | Set `NEMOCLAW_GATEWAY_RUNTIME=podman` before standard onboarding. The provider requires a qualified current-user Podman socket, rootless service, cgroups v2, bridge networking and DNS, `lsof` on `PATH` for gateway listener verification, NVIDIA CDI for GPU use, and exact managed-image receipts. NemoClaw does not install `lsof`; install it through the host's package-management policy before retrying onboarding. Full native E2E qualification currently runs on Linux amd64; the managed-image contract accepts `linux/amd64` and `linux/arm64` receipts. The WSL Arm64 Ollama capacity proof is covered by provider-contract and onboarding tests; physical end-to-end validation remains pending. Managed local vLLM install/start remains Docker-backed and is unavailable without Docker. Custom `--from` Dockerfiles and read-only host mounts are not qualified. |
...
| Other Windows-on-ARM GPU paths | Unsupported | Windows-on-ARM CPU paths run under WSL2 'tested with limitations'. The qualifying Windows WSL N1x paths remain pending dedicated CI and physical end-to-end validation. A matching provider proof can establish GPU availability for multiple plausible NVIDIA rows, but it does not qualify a generic host for automatic large-model selection. Unqualified providers, mismatched rows, implausible names, or failed bounded CUDA proof remain rejected. See closed issue #4565 and #12073. |
