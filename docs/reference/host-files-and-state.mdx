...
For the rows below, `<selected-state-root>` is `~/.nemoclaw/` for port `8080` or `~/.nemoclaw/gateways/<port>/` for another port.
...
| `~/.nemoclaw/credentials.json` | Legacy plaintext credential file from earlier releases. Onboarding stages allowlisted values and registers them with the OpenShell gateway. After verified migration, it removes only unchanged migrated entries and preserves all others. It securely deletes the file when no entries remain. See [Credential Storage](../security/credential-storage) for retained-file and empty-file cleanup behavior. | No; run `$$nemoclaw onboard` to complete migration and cleanup so you do not lose a credential that is not yet registered with the gateway. |
...
| `<selected-state-root>/onboard-session.json` | Resume or incomplete-state record for an onboarding attempt that did not complete. It can contain secret-free external component activation evidence bound to the sandbox identity fingerprint and lifecycle generation. | Only when you intentionally want to discard the failed session and `externalComponentActivation` is absent. Prefer `$$nemoclaw onboard --fresh` when available. If that field is present, preserve the file, sandbox registry, and sandbox for an identity-verified recovery decision. Version 1 does not support resume or recovery for that activation state. |
...
`--delete-models` deletes every model in the local Ollama inventory and all non-credential data in the current user's shared Hugging Face cache.
