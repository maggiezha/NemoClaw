...
Onboarding under a non-default `NEMOCLAW_GATEWAY_PORT` registers the sandbox on gateway `nemoclaw-<port>` and stores its registry and state under `~/.nemoclaw/gateways/<port>/`. Every later command reads the state root that its own `NEMOCLAW_GATEWAY_PORT` selects. A command without that variable uses port `8080` unless the installer recorded an automatic alternate-port selection; in that case, the CLI restores the recorded port. Clearing an explicitly supplied variable does not move an existing sandbox back to the default port.
...
### Hermes Portable Port Forward Recovery Fails

If Hermes Portable host-forward recovery fails, use the reported cause before retrying.
Every failure prevents launch-readiness publication.

| Reported Cause | Meaning and Action |
| --- | --- |
| `Recorded host port <port> is occupied` | Another sandbox or listener owns the port. NemoClaw changes no forwards. Inspect `openshell forward list` and the host listener. Stop it only after you confirm ownership. |
| OpenShell `forward list` failed | NemoClaw could not prove the recorded forward state. Restore OpenShell access before you retry. |
| OpenShell `forward list` returned malformed or ambiguous state | Inspect the complete forward list before you retry. |
| NemoClaw could not resolve the required recorded host ports | Inspect the sandbox registry and launch-forward configuration before you retry. NemoClaw changes no forwards. |
| NemoClaw could not verify whether recorded host port `<port>` is reachable | Inspect the listener and recorded forward state before you retry. |
| The required recorded host forwards did not become healthy before the recovery deadline | Inspect the recorded forward state before another probe or launch. |
| NemoClaw could not confirm that OpenShell forward `start` or `stop` completed | Inspect the named port and recorded forward state before you retry. |

If recovery cannot prove rollback or reports authority drift, do not run another probe or launch.
Inspect the recorded forward state or current Portable authority first.
