...
Set `NEMOCLAW_GATEWAY_RUNTIME=podman` before onboarding and make sure the current-user Podman service and `lsof` on `PATH` are available. NemoClaw does not install `lsof`; install it through the WSL distribution's package-management policy before retrying onboarding. NVIDIA CDI is additionally required for sandbox GPU passthrough and the N1x CUDA capacity proof.
...
A Windows on Arm host with multiple GPUs uses the provider proof to validate every reported device and its aggregate capacity, but that proof does not qualify a generic host for automatic 30B or 35B selection.
Only the N1x qualification described above can select `qwen3.6:35b`; other Windows on Arm GPUs remain compute-constrained and select `qwen3.5:9b`.
<AgentOnly variant="openclaw">
