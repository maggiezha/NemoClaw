...
describe(
  "onboard Anthropic credential retry UX",
  {
  ...
  () => {
    it("lets users re-enter an Anthropic API key after authorization failure (#6289)", () => {
      ...
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "nemoclaw-onboard-anthropic-auth-retry-"),
      );
...
`;
      ...
      assert.ok(
        payload.messages.some((message: string) => CREDENTIAL_RETRY_PROMPT_RE.test(message)),
      );
